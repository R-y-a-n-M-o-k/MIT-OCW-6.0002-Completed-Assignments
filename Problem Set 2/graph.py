...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        #check if nodes are valid first
        if (edge.src not in self.nodes) or (edge.dest not in self.nodes):
            raise ValueError('src and dest are not nodes in the graph')
        
        #if no existing edges, create edge list
        elif edge.src not in self.edges:
